    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={handleSubmit} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((article) => (
            <span
              onClick={() => SetSelected(article)}
              key={article.length}
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selected.text === article.text ? "bg-blue-50 text-blue-700" : "text-gray-500 border-gray-300"}`}
            >{article.text}
