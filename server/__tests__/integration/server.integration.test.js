...
const seedCreation = async (overRide = {}) => {
    const [creation] = await pgsql`INSERT INTO creations (user_id, prompt, content, type, publish, likes)
    VALUES (
      ${TEST_USER_ID},
...
describe("Integration Test", () => {
    ...
    describe("DATABASE CONNECTION", () => {
        ...
        it("should have a creation table", async () => {
            const res = await request(app).post("/api/ai/generate-article").send({ prompt: "Write about integration test", length: "short", publish: false })

            expect(res.status).toBe(200)
            expect(res.body.success).toBe(true)
            const [saved] = await pgsql`SELECT * FROM creations WHERE user_id = ${TEST_USER_ID} AND type = 'article';`
        ...
        it("should persist article with publish = true to real db", async () => {
            const res = await request(app).post("/api/ai/generate-article").send({ prompt: "Published article test", length: "short", publish: true })
    ...
    describe("POST /api/ai/generate-image", () => {
        it("should generate image and persist it to real db", async () => {
            ...
            const res = await request(app).post("/api/ai/generate-image").send({ prompt: "A beautiful sunset over the ocean", publish: true })

            expect(res.status).toBe(200)
            expect(res.body.success).toBe(true)
            expect(res.body.content).toBe("https://cloudinary.com/mock-image.png")

            const [saved] = await pgsql`SELECT * FROM creations WHERE user_id = ${TEST_USER_ID} and type = 'image';`
            expect(saved).toBeDefined()
        ...
        it("should return 400 if prompt is missing", async () => {
            const res = await request(app).post("/api/ai/generate-image").send({ publish: false })
    ...
    describe("POST /api/user/toggle-like-creations", () => {
        it("should like a creation and persist to real DB", async () => {
            ...
            const res = await request(app).post("/api/user/toggle-like-creations").send({ id: creation.id })
            expect(res.status).toBe(200)
            expect(res.body.success).toBe(true)
            const [updated] = await pgsql`SELECT * FROM creations WHERE id = ${creation.id}`
        ...
        it("should return 400 if creation does not exist in DB", async () => {
            const res = await request(app).post("/api/user/toggle-like-creations").send({ id: 99999 })
            expect(res.status).toBe(400)
            expect(res.body.success).toBe(false)
    ...
    describe("POST /api/ai/generate-blog", () => {
        it("should generate blog title and persist to real db", async () => {
            const res = await request(app).post("/api/ai/generate-blog").send({ prompt: "Write a blog title about AI trends" })
            console.log("blog-title-body", res.body)
            expect(res.status).toBe(200)
            expect(res.body.success).toBe(true)
            expect(res.body.content).toBeDefined()

            const [saved] = await pgsql`SELECT * FROM creations WHERE user_id = ${TEST_USER_ID} AND type = 'blog-title';`
            expect(saved).toBeDefined()
    ...
    describe("POST /api/ai/remove-image-background", () => {
        it("should remove background and persist to real DB", async () => {
            const res = await request(app).post("/api/ai/remove-image-background").attach("image", Buffer.from("mock image content"), {
                filename: "test.png",
        ...
        it("should return 400 if no image is uploaded", async () => {
            const res = await request(app).post("/api/ai/remove-image-background").send({})
            expect(res.status).toBe(400)
            expect(res.body.success).toBe(false)
    ...
    describe("POST /api/ai/remove-image-object", () => {
        it("should remove object and persist to real db", async () => {
            const res = await request(app).post("/api/ai/remove-image-object").field("object", "car").attach("image", Buffer.from("mock image content"), {
                filename: "test.png",
                contentType: "image/png"
            })
            expect(res.status).toBe(200)
        ...
        it("should return 400 if image is missing", async () => {
            const res = await request(app).post("/api/ai/remove-image-object").send({ object: "car" })
            expect(res.status).toBe(400)
            expect(res.body.success).toBe(false)
        })
        it("should return 400 if object is missing", async () => {
            const res = await request(app).post("/api/ai/remove-image-object").attach("image", Buffer.from("mock image content"), {
                filename: "test.png",
                contentType: "image/png"
            })
            expect(res.status).toBe(400)
    ...
    describe("POST /api/ai/resume-review", () => {
        it("should review resume and persist to real db", async () => {
            const res = await request(app).post("/api/ai/resume-review").attach("resume", Buffer.from("mock pdf content"), {
                filename: "resume.pdf",
            ...
            const [saved] = await pgsql`SELECT * FROM creations WHERE user_id = ${TEST_USER_ID} AND type = 'resume-review';`
            expect(saved).toBeDefined()
            expect(saved.user_id).toBe(TEST_USER_ID)
        })
        it("should return error if resume exceeds 5mb", async () => {
            const largeBuffer = Buffer.alloc(6 * 1024 * 1024)
            const res = await request(app).post("/api/ai/resume-review").attach("resume", largeBuffer, {
