    ...
    default: {
        post: jest.fn().mockImplementation((url, config, data) => {
            if (!process.env.CLIPDROP_API_KEY) {
                return Promise.reject({ response: { status: 403, data: "Invalid API response" } })
            }
...
jest.unstable_mockModule("cloudinary", () => ({
    v2: {
        uploader: {
            upload: jest.fn().mockResolvedValue({ secure_url: "https://cloudinary.com/mock-image.png", public_id: "mock_public_id" })
        },
...
jest.unstable_mockModule("pdf-parse", () => ({
    PDFParse: jest.fn().mockResolvedValue({ text: "Mock resume content withs skilss and experience" })
}))
...
describe("AI controller Test", () => {
    ...
    describe("POST /api/ai/remove-image-object", () => {
        it("should return 400 if no image or object is provided", async () => {
            const res = await request(app).post("/api/ai/remove-image-object").send({})
        ...
        it("should return 400 if object is missing but image is provided", async () => {
            const res = await request(app).post("/api/ai/remove-image-object").attach("image", Buffer.from("mock image content"), { filename: "test.png", contentType: "image/png" })
            expect(res.status).toBe(400)
            expect(res.body.success).toBe(false)
